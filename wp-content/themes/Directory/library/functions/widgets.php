...
if(!class_exists('supreme_advertisements'))
{
	class supreme_advertisements extends WP_Widget
	{
		...
		{
			//Constructor
			$widget_ops = array('classname'  => 'widget Templatic Advertisements','description'=> __('Use this widget to display ads by pasting HTML, JavaScript or IFrame code.',ADMINDOMAIN) );
			$this->WP_Widget('supreme_advertisements', __('T &rarr; Advertisements',ADMINDOMAIN), $widget_ops);
		}
		function widget($args, $instance)
		{
			...
			$ads = empty($instance['ads']) ? '<a href="http://www.templatic.com"><img src="'.get_template_directory_uri().'/images/dummy/top-ad.jpg" /></a>' : $instance['ads'];
			echo $before_widget;
			if( $title <> "" )
			{
...
<div class="advertisements"> <?php echo $ads; ?> </div>
<?php
			echo $after_widget;
		}
		...
		function form($instance)
		{
			...
			$ads   = ($instance['ads']) ? trim($instance['ads']) : '';
			?>
<p>
...
<p>
  <label for="<?php echo $this->get_field_id('ads'); ?>">
    <?php echo __('Advertisement code <small>(ex.&lt;a href="#"&gt;&lt;img src="http://templatic.com/banner.png" /&gt;&lt;/a&gt; and google ads code here )</small>',ADMINDOMAIN);?>
    :
    <textarea class="widefat" rows="10" id="<?php echo $this->get_field_id('ads'); ?>" name="<?php echo $this->get_field_name('ads'); ?>"><?php echo esc_attr($ads); ?></textarea>
  </label>
</p>
<?php
		}
...
if(!class_exists('supreme_flickr_Widget'))
{
	class supreme_flickr_Widget extends WP_Widget
	{
		...
		{
			//Constructor
			$widget_ops = array('classname'  => 'widget Flicker Photos ','description'=> __('Showcase your Flickr photos using this simple widget. Works best in sidebar areas.',ADMINDOMAIN) );
		...
		function widget($args, $instance)
		{
			// prints the widget
			extract($args, EXTR_SKIP);
			$title = empty($instance['title']) ? __("Photos From Flickr",THEME_DOMAIN) : apply_filters('widget_title', $instance['title']);
		...
		<div class="Flicker"><?php
				...
				if($title != ""):?>
				<h3 class="widget-title"> <?php echo sprintf(__('%s',THEME_DOMAIN),$title);?> </h3>
				<?php endif;
				if($desc != "")
				{
...
class templatic_text extends WP_Widget
{
	...
	{
		//Constructor
		$widget_ops = array('classname'    => 'widget Templatic Text templatic_text','description'  => __('Arbitrary text or HTML',ADMINDOMAIN),'before_widget'=>'<div class="column_wrap">' );
...
if(!class_exists('supreme_google_map')){
	class supreme_google_map extends WP_Widget
	{
		...
		function widget($args, $instance)
		{
			// prints the widget
			extract($args, EXTR_SKIP);
			$title = empty($instance['title']) ? __("Find Us On Map",THEME_DOMAIN) : apply_filters('widget_title', $instance['title']);
...
if(!class_exists('supreme_social_media'))
{
	class supreme_social_media extends WP_Widget
	{
		...
		{
			//Constructor
			$widget_ops = array('classname'  => 'widget social_media','description'=> apply_filters('supreme_social_media_description',__('Add icons and links to your social media accounts. Works in header, footer, main content and sidebar widget areas.',ADMINDOMAIN)) );
...
if(!class_exists('supreme_subscriber_widget'))
{
	class supreme_subscriber_widget extends WP_Widget
	{
		...
		{
			//Constructor
			$widget_ops = array('classname'  => 'widget Newsletter subscribe','description'=> apply_filters('supreme_subscriber_widget_title',__('Shows a subscribe box with which users can subscribe to your newsletter. Works best in main content, subsidiary and footer areas.',ADMINDOMAIN) ));
		...
		function widget($args, $instance)
		{
			...
			$title = empty($instance['title']) ? __("Subscribe To Newsletter",THEME_DOMAIN) : apply_filters('widget_title', $instance['title']);
			$text = empty($instance['text']) ? __("Subscribe to get our latest news",THEME_DOMAIN) : apply_filters('widget_text', $instance['text']);
...
{echo 'display:none';};?>">
  <label for="<?php echo $this->get_field_id('aweber_list_name'); ?>">
    <?php echo __('List Name:',ADMINDOMAIN);?>
...
if(!class_exists('supreme_testimonials_widget'))
{
	class supreme_testimonials_widget extends WP_Widget
	{
		...
		{
			//Constructor
			$widget_ops = array('classname'  => 'widget testimonials','description'=> 'Display a set of sliding testimonials. Works best in sidebar areas.',ADMINDOMAIN);
...
<div class="testimonials">
  <?php
				if($title)
				{
					...
					?>
  <h3 class="widget-title i_testimonials"> <span> <?php echo $title; ?> </span> </h3>
  <?php
				}?>
...
<p>
  <?php
				...
				?>
  <a
					href="javascript:void(0);" id="removetButton" class="removeButton" type="button" onclick="remove_tfields();">- Remove </a> </p>
<p>
...
function add_testimonial_submit_button($instance,$text_quotetext,$text_author)
{
	?>
<a
		href="javascript:void(0);" id="addtButton" class="addButton" type="button" onclick="add_tfields('<?php echo $text_author; ?>','<?php echo $text_quotetext; ?>');">+ Add more </a>
<?php
}
/*-----------------------------------------------
	...
	class supreme_banner_slider extends WP_Widget
	{
		function __construct()
		{
			parent::__construct(
				...
				__('T &rarr; Homepage Slider',ADMINDOMAIN), // Name
				array('description'=> __('Display images from a specific category or a collection of static images. The widget works best in the Homepage Slider area.',ADMINDOMAIN)) // Args
			);
		}
		function widget($args, $instance)
		{
		...
		jQuery(window).load(function(){
			...
			<?php if(isset($is_Carousel) && $is_Carousel==1)
			{
				...
				itemMargin: <?php if($min_item!="")
				{
					echo $min_item;
				}else echo '0'?>,                  //{NEW} Integer: Margin between carousel items.
				...
				maxItems: <?php echo $max_items;?>,                    //{NEW} Integer: Maximum number of carousel items that should be visible. Items will resize fluidly when above this limit.
...
<?php
		...
		<div class="flexslider clearfix <?php echo $animation_class.$class.$custom_class_name.$is_Carousel; ?>">
		  <?php	if($title){ ?><h3 class="widget-title"> <?php echo $title; ?> </h3>
		  <?php	} 
				if(function_exists('icl_register_string'))
				{
	   ...
	   <div class="slides_container clearfix">
		...
		<?php
						if($s1[$i] != ""): ?>
		<li class="post_img" > <a href="<?php echo $s1_title_link[$i]; ?>" target="_blank"> <img src="<?php echo $s1[$i]; ?>" alt="" />
        <?php	if($s1_title[$i] != ""):?>
			<h2><?php
									if(function_exists('icl_register_string'))
									{
						...
						if( $slide->have_posts() )
						{
							...
							?>
								...
								if(current_theme_supports('slider_thumb_image') && $custom_banner_temp == ''){
									$thumb_image_size = apply_filters('slider_thumb_image','thumbnail');?> data-thumb = "<?php get_the_image( array('size' => $thumb_image_size,'echo' => "false" ,'default_image'=>get_template_directory_uri().'/images/noimage.jpg'));?>" <?php } ?> > <?php
							} ?>
        <!-- post start -->
        <div class="post_list">
        <?php	if(get_post_format($post->ID) == 'image'): ?>
        <div class="post_img"><?php if($large_image != ""){ echo $large_image; } ?></div>
        <?php	else: ?>
		<div
			<?php
			if($flag == 1)
			{
				?> class="post_img" style="width:<?php echo $width."px"; ?>"<?php
			} ?>>
		<?php
			if($post_images != "")
			{
...
<?php
		...
		function form($instance)
		{
			...
			$slider_post_type  = strip_tags($instance['slider_post_type']);
...
{?>display:block;<?php }?>">
 <p>
		 	<label for="<?php echo $this->get_field_id('slider_post_type');  ?>"></label>
		 	<select class="widefat" name="<?php echo $this->get_field_name('slider_post_type'); ?>" id="<?php  echo $this->get_field_id('slider_post_type'); ?>" onchange="tmpl_change_category('<?php echo $this->get_field_id('post_type_category'); ?>',this.value);" >
		 		<?php
               $all_post_types = get_option("templatic_custom_post");
               foreach($all_post_types as $key=>$post_types){
                    ?>
                    <option value="<?php echo $key;  ?>" <?php if($slider_post_type ==$key){ echo "selected=selected"; }?>><?php echo esc_attr(ucfirst($post_types['label']));?></option>
                    <?php
               }
               ?>	
		 	</select>
  ...
  <p>
    <label for="<?php echo $this->get_field_id('post_type');?>" >
      ...
      <select id="<?php echo $this->get_field_id('post_type_category'); ?>" name="<?php echo $this->get_field_name('post_type'); ?>[]" class="widefat"  multiple="multiple" size="8">
        <?php $taxonomies = get_taxonomies( array( 'public' => true,'object_type' =>array($slider_post_type)  ), 'objects' );
						foreach ( $taxonomies as $taxonomy ) {	
							$query_label = '';
  ...
  <p>
    <label for="<?php echo $this->get_field_id('content'); ?>">
      <input <?php if($content){ echo "checked=checked"; }?> id="<?php echo $this->get_field_id('content'); ?>" name="<?php echo $this->get_field_name('content'); ?>" type="checkbox" value="1"/>
      <b><?php echo __('Enable Post Excerpt In Slider',ADMINDOMAIN);?></b> </label>
  </p>
  <?php } ?>
</div>
<p> <br/>
...
{?>display:none;<?php }?>">
  <div id="TextBoxesGroup" class="TextBoxesGroup">
	...
	<a	href="javascript:void(0);" id="addButton" class="addButton" onclick="add_textbox('<?php echo $textbox_name;?>','<?php echo $textbox_title_link; ?>','<?php echo $textbox_title;?>');"> <?php echo __('+Add more',ADMINDOMAIN); ?> </a> &nbsp; | &nbsp; <a
					href="javascript:void(0);" id="removeButton" class="removeButton" onclick="remove_textbox();"><?php echo __('-Remove',ADMINDOMAIN); ?> </a> </div>
<?php	}
	}
	/*
	* templatic Slider widget init
...
if(!class_exists('supreme_contact_widget'))
{
	class supreme_contact_widget extends WP_Widget
	{
		...
		{
			//Constructor
			$widget_ops = array('classname'  => 'Contact Us','description'=> apply_filters('templ_contact_widget_desc_filter',__('A simple contact form visitors can use to get in touch with you. Works best in sidebar areas.',ADMINDOMAIN)) );
		...
		function widget($args, $instance)
		{
			// prints the widget
			extract($args, EXTR_SKIP);
			$title = empty($instance['title']) ? __("Contact Us",ADMINDOMAIN) : apply_filters('widget_title', $instance['title']);
	    ...
	    <?php	if($title)
					{	?>
	    <h3 class="widget-title"> <?php echo sprintf(__('%s',ADMINDOMAIN), $title);?> </h3>
	    <?php	}
  ...
  <?php	if(isset($_REQUEST['capt']) && $_REQUEST['capt'] == 'captch' && !isset($_REQUEST['message'])){
						$blank_field = $a['no_response_error'];
						$incorrect_field = $a['incorrect_response_error'];
	?>
  <p class="error_msg">
    <?php echo sprintf(__('%s',THEME_DOMAIN),$incorrect_field);?>
  </p>
  ...
  <p class="success_msg"> <?php _e('Thank you, your information is sent successfully.',THEME_DOMAIN); ?> </p>
  <?php }elseif(isset($_REQUEST['err']) && $_REQUEST['err'] == 'empty'){	?>
  ...
  <form action="<?php echo $_SERVER['REQUEST_URI'];?>" method="post" id="contact_widget_frm" name="contact_frm" class="wpcf7-form">
    ...
    <div class="form_row ">
      ...
      <span id="widget_your_name_Info" class="error">
      <?php echo " "; ?>
      </span> </div>
    <div class="form_row ">
      ...
      <?php 
							if($captcha == 1)
							{
								...
								{							
									...
									echo '<label class="recaptcha_claim">'; _e('Verify words',THEME_DOMAIN); echo ' <span>*</span></label>';
...
<?php
		...
		function form($instance)
		{
			...
			$title = (strip_tags($instance['title'])) ? strip_tags($instance['title']) : __("Contact Us",ADMINDOMAIN);
...
if(!class_exists('supreme_recent_post'))
{
	class supreme_recent_post extends WP_Widget
	{
		...
		{
			//Constructor
			$widget_ops = array('classname'  => 'widget Templatic listing_post','description'=> __('Showcase posts from a particular category and post type. Use in main content, subsidiary and sidebar areas.',ADMINDOMAIN) );
  ...
  <figure class="post_fig">
    ...
    <a href="<?php echo get_permalink(); ?>" title="<?php the_title_attribute( 'echo=1' ); ?>" rel="bookmark" class="featured-image-link"><img src="<?php echo $post_image; ?>" width="<?php echo $width;?>" height="<?php echo $height;?>" alt="<?php the_title_attribute( 'echo=1' ); ?>" <?php echo $style; ?>/></a> </figure>
  <?php				
					//echo $image = get_the_image( array( 'echo' => false,'','size' =>$instance['image_size'],'default_image'=>get_template_directory_uri()."/images/noimage.jpg" ) );
				endif;
...
if(!class_exists('supreme_popular_post'))
{
	class supreme_popular_post extends WP_Widget
	{
		...
		{
			//Constructor
			$widget_ops = array('classname'  => 'widget Templatic Popular Posts Widget ','description'=> __('Show a list of popular posts from a particular post type. Popularity is determined by total/daily views or comments. Works best in sidebar areas.',ADMINDOMAIN) );
...
<?php
			...
			{
				?>
<div class="popular_post templatic_popular_post_technews">
...
<?php
			...
			?>
<div class="popular_post templatic_popular_post_technews">
  <ul class="listingview clearfix list" id="list">
  </ul>
</div>
<?php
			if( @$pagination_position != 1 && !empty($totalpost) && count($totalpost) > 0 )
...
if(!class_exists('supreme_recent_review'))
{
	class supreme_recent_review extends WP_Widget
	{
		...
		{
			//Constructor
			$widget_ops = array('classname'  => 'widget recent_reviews Recent Review','description'=> __('Display recent reviews from a specific post type. Works best in sidebar areas.',ADMINDOMAIN) );
	...
	function recent_review_comments($g_size = 30, $no_comments = 10, $comment_lenth = 60, $show_pass_post = false,$post_type = 'post',$title = '')
	{
		...
		if($comments)
		{
			...
			{ 
				...
				echo "<div class='rating'>".draw_rating_star_plugin($post_rating)."</div>";
				endif;
				...
				if(isset($theme_options['templatic_excerpt_link']))
				{
					$read_more = $theme_options['templatic_excerpt_link'];
				}
				else
				{
					$read_more = __('read more',THEME_DOMAIN);
				}
...
add_action('display_multi_rating','display_multi_rating');
function display_multi_rating($comment_id){
