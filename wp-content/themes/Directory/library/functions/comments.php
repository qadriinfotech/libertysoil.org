...
function supreme_comment_form_args( $args ) {
	...
	$req = ( ( get_option( 'require_name_email' ) ) ? ' <span class="required">' . __( '*', THEME_DOMAIN ) . '</span> ' : '' );
	$input_class = ( ( get_option( 'require_name_email' ) ) ? ' req' : '' );
	/* Sets up the default comment form fields. */
	$fields = array(
		'author' => '<p class="form-author' . esc_attr( $input_class ) . '"><label for="author">' . __( 'Name', THEME_DOMAIN ) . $req . '</label> <input type="text" class="text-input" name="author" id="author" value="' . esc_attr( $commenter['comment_author'] ) . '" size="40" /></p>',
		'email' => '<p class="form-email' . esc_attr( $input_class ) . '"><label for="email">' . __( 'Email', THEME_DOMAIN ) . $req . '</label> <input type="text" class="text-input" name="email" id="email" value="' . esc_attr( $commenter['comment_author_email'] ) . '" size="40" /></p>',
		'url' => '<p class="form-url"><label for="url">' . __( 'Website', THEME_DOMAIN ) . '</label><input type="text" class="text-input" name="url" id="url" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="40" /></p>'
	);
	...
	$args = array(
		...
		'comment_field' => '<p class="form-textarea req"><label for="comment">' . __( 'Comment', THEME_DOMAIN ) . '</label><textarea name="comment" id="comment" cols="60" rows="10"></textarea></p>',
		'must_log_in' => '<p class="alert">' . sprintf( __( 'You must be <a href="%1$s" title="Log in">logged in</a> to post a comment.', THEME_DOMAIN ), $login_url ) . '</p><!-- .alert -->',
		'logged_in_as' => '<p class="log-in-out">' . __( 'Logged in as', THEME_DOMAIN )." ".'<a href="'.get_author_posts_url( get_current_user_id() ).'" title="'. esc_attr( $user_identity ).'">'. esc_attr( $user_identity ).'</a>. <a href="' . wp_logout_url( get_permalink() ) . '" title="' .__( 'Log out of this account', THEME_DOMAIN ). '">' . __( 'Log out &raquo;', THEME_DOMAIN ) . '</a></p><!-- .log-in-out -->',
		'comment_notes_before' => '',
		'comment_notes_after' => '',
		'id_form' => 'commentform',
		'id_submit' => 'submit',
		'title_reply' => (get_post_type()=='post')? __( 'Leave a Reply', THEME_DOMAIN ):__( 'Add a Review', THEME_DOMAIN ) ,
		'title_reply_to' => (get_post_type()=='post')?__( 'Leave a Reply to %s', THEME_DOMAIN ) :__( 'Add a Review to %s', THEME_DOMAIN ) ,
		'cancel_reply_link' => __( 'Click here to cancel reply.', THEME_DOMAIN ),
